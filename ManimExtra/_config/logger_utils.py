...
Manim's logger can be accessed as ``ManimExtra.logger``, or as
``logging.getLogger("ManimExtra")``, once the library has been imported.  Manim also
...
def make_logger(
    ...
    """Make the ManimExtra logger and console.
    ...
    :class:`logging.Logger`, :class:`rich.Console`, :class:`rich.Console`
        The ManimExtra logger and consoles. The first console outputs
    ...
    # finally, the logger
    logger = logging.getLogger("ManimExtra")
...
def parse_theme(parser: configparser.ConfigParser) -> Theme:
    ...
    -------
    :class:`rich.Theme`
        The rich theme to be used by the ManimExtra logger.
...
def set_file_logger(scene_name: str, module_name: str, log_dir: Path) -> None:
    """Add a file handler to ManimExtra logger.
    ...
    # appear if scene name was provided when ManimExtra was called.
