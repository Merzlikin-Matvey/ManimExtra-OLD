...
def config_file_paths() -> list[Path]:
    ...
    When ManimExtra is first imported, it processes any ``.cfg`` files it finds.  This
    ...
    The library-wide config file determines ManimExtra's default behavior.  The
    user-wide config file is stored in the user's home folder, and determines
    the behavior of ManimExtra whenever the user invokes it from anywhere in the
    ...
    if sys.platform.startswith("win32"):
        user_wide = Path.home() / "AppData" / "Roaming" / "Manim" / "ManimExtra.cfg"
    else:
        user_wide = Path.home() / ".config" / "ManimExtra" / "ManimExtra.cfg"
    folder_wide = Path("ManimExtra.cfg")
...
class ManimConfig(MutableMapping):
    ...
    .. code-block:: pycon
        ...
        >>> from ManimExtra import config as global_config
    ...
    def digest_args(self, args: argparse.Namespace) -> ManimConfig:
        ...
        if nflag:
            ...
            try:
                ...
                logging.getLogger("ManimExtra").info(
    ...
    def verbosity(self, val: str) -> None:
        ...
        logging.getLogger("ManimExtra").setLevel(val)
    ...
    def format(self, val: str) -> None:
        ...
        if self.format == "webm":
            logging.getLogger("ManimExtra").warning(
    ...
    def get_dir(self, key: str, **kwargs: str) -> Path:
        ...
        .. code-block:: pycon
            ...
            >>> from ManimExtra import config as globalconfig
