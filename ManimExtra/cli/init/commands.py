...
Manim's init subcommand is accessed in the command-line interface via ``ManimExtra
...
def update_cfg(cfg_dict: dict, project_cfg_path: Path):
    """Updates the ManimExtra.cfg file after reading it from the project_cfg_path.

    Parameters
    ----------
    cfg_dict
        values used to update ManimExtra.cfg found project_cfg_path.
    project_cfg_path
        Path of ManimExtra.cfg file.
    """
...
def project(default_settings, **args):
    ...
    if project_name.is_dir():
        ...
        project_name.mkdir()
        new_cfg = {}
        new_cfg_path = Path.resolve(project_name / "ManimExtra.cfg")
