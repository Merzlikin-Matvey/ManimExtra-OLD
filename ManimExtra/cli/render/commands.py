...
``ManimExtra``, but can be more explicitly accessed with ``ManimExtra render``. Here you
...
def render(
    **args,
    ...
    if config.notify_outdated_version:
        ...
        warn_prompt = "Cannot check if latest release of ManimExtra is installed"
        req_info = {}
        ...
        try:
            ...
            if stable != __version__:
                console.print(
                    f"You are using ManimExtra version [red]v{__version__}[/red], but version [green]v{stable}[/green] is available.",
                )
                console.print(
                    "You should consider upgrading via [yellow]pip install -U ManimExtra[/yellow]",
                )
