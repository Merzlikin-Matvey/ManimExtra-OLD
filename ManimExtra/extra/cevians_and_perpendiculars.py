...
from ManimExtra.mobject.geometry.line import Angle
from ManimExtra.constants import PI
...
class Bisector(Cevian):
    ...
    def bisected_angles(self, **kwargs) -> VGroup(Angle, Angle):
        return VGroup(
            Angle().from_three_points(self.extra_vertex_1, self.general_vertex, self.foot, **kwargs),
            Angle().from_three_points(self.extra_vertex_2, self.general_vertex, self.foot, **kwargs)
        )
...
class Median(Cevian):
    ...
    def equal_segments(self, **kwargs) -> VGroup(Line, Line):
        return VGroup(
