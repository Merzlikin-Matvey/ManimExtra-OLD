...
from ManimExtra import __version__
...
class SceneFileWriter:
    ...
    def begin_animation(self, allow_write: bool = False, file_path=None):
        """
        Used internally by ManimExtra to stream the animation to FFMPEG for
    ...
    def clean_cache(self):
        ...
        if len(cached_partial_movies) > config["max_files_cached"]:
            ...
            logger.info(
                f"The partial movie directory is full (> {config['max_files_cached']} files). Therefore, ManimExtra has removed the {number_files_to_delete} oldest file(s)."
