...
render scenes that are defined within doctests, for example::
    ...
    .. manim:: DirectiveDoctestExample
        ...
        >>> from ManimExtra import Create, Dot, RED, Scene
        >>> dot = Dot(color=RED)
        >>> dot.color
        <Color #fc6255>
...
from ManimExtra import QUALITIES

classnamedict = {}


class SkipManimNode(nodes.Admonition, nodes.Element):
    """Auxiliary node class that is used when the ``skip-ManimExtra`` tag is present
    or ``.pot`` files are being built.

    Skips rendering the ManimExtra directive and outputs a placeholder instead.
    """
...
def visit(self, node, name=""):
    ...
    if not isinstance(node[0], nodes.title):
        node.insert(0, nodes.title("skip-ManimExtra", "Example Placeholder"))
...
class ManimDirective(Directive):
    r"""The ManimExtra directive, rendering videos while building
    the documentation.
    ...
    def run(self):
        # Rendering is skipped if the tag skip-ManimExtra is present,
        # or if we are making the pot-files
        should_skip = (
            "skip-ManimExtra" in self.state.document.settings.env.app.builder.tags.tags
...
def _write_rendering_stats(scene_name, run_time, file_name):
    with rendering_times_file_path.open("a") as file:
        csv.writer(file).writerow(
            [
                re.sub(r"^(reference\/)|(ManimExtra\.)", "", file_name),
                scene_name,
                "%.3f" % run_time,
            ],
        )
...
def setup(app):
    ...
    app.add_directive("ManimExtra", ManimDirective)
...
.. raw:: html
    ...
    <div id="{{ clsname_lowercase }}" class="admonition admonition-ManimExtra-example">
